...
template <typename Ty_> class FixedSizeStack {
public:
  ...
  const Ty_ &operator[](std::size_t index) { return m_stack[index]; }
...
public:
  ...
  void operator<<(const Token &token) {
    if ((token.m_token_class == TokenClass::REGULAR) ||
        (token.m_token_class == TokenClass::STRING)) {
...
public:
  ...
  ANTLRToken(ANTLRToken &&other) {
    ...
    std::copy(other.m_payload.begin(), other.m_payload.end(),
  ...
  ANTLRToken(std::size_t idx, token_t token_type, std::size_t start_line,
             std::size_t start_column, std::size_t end_line,
             std::size_t end_column, std::string_view payload)
      : m_idx(idx), m_token_type(token_type), m_start_line(start_line),
        m_start_column(start_column), m_end_line(end_line),
  ...
  std::int16_t channel() const { return DEFAULT_CHANNEL; }

  std::size_t tokenIndex() const { return m_idx; }

  std::size_t line() const { return m_start_line; }

  std::size_t column() const { return m_start_column - 1; }

  token_t type() const { return m_token_type; }
  ...
  std::optional<std::size_t> start() const { return std::nullopt; }

  std::optional<std::size_t> stop() const { return std::nullopt; }
...
public:
  ...
  ANTLRToken create(std::string source, ANTLRToken::token_t type,
                    std::string text, std::int64_t channel, std::int64_t start,
                    std::int64_t stop, std::int64_t line, std::int64_t column) {
...
class TokenSource {
public:
  TokenSource() { m_token_factory = TokenFactory(); }

  const TokenFactory &getFactory() const { return m_token_factory; }
...
public:
    ...
    if (_numMarkers == 0) { // can we release buffer?
      if (_p > 0) {
        ...
        _tokens.erase(_tokens.begin(),
                      _tokens.begin() + static_cast<std::size_t>(_p));
        _p = 0;
      }
      _lastTokenBufferStart = _lastToken;
    }
  ...
  std::size_t getIndex() const { return _currentTokenIndex; }

  std::size_t getBufferStartIndex() const { return _currentTokenIndex - _p; }
  ...
  void seek(std::size_t index) {
    ...
    if (bufferStartIndex > index) {
      throw IllegalArgumentException(
          std::string("cannot seek to negative index ") +
          std::to_string(index));
    }
    ...
    if (i >= _tokens.size()) {
      throw UnsupportedOperationException(
          std::string("seek to index outside buffer: ") +
  ...
  bool eof() const { return ::feof(m_file) != 0; }
  ...
  const TokenSource &getTokenSource() const { return m_source; }
...
protected:
  ...
  void sync(std::size_t want) {
    std::size_t need = (static_cast<std::size_t>(_p) + want - 1) -
                       static_cast<std::size_t>(_tokens.size()) + 1;
    if (need > 0) {

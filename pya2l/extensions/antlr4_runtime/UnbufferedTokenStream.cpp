...
Token *UnbufferedTokenStream::get(size_t i) const {  // get absolute index
    ...
    if (i < bufferStartIndex || i >= bufferStartIndex + _tokens.size()) {
        throw IndexOutOfBoundsException(
            std::string("get(") + std::to_string(i) + std::string(") outside buffer: ") + std::to_string(bufferStartIndex) +
...
void UnbufferedTokenStream::sync(ssize_t want) {
    ssize_t need =
        (static_cast<ssize_t>(_p) + want - 1) - static_cast<ssize_t>(_tokens.size()) + 1;  // how many more elements we need?
    if (need > 0) {
...
void UnbufferedTokenStream::seek(size_t index) {  // seek to absolute index
    ...
    if (i >= _tokens.size()) {
        throw UnsupportedOperationException(
            std::string("seek to index outside buffer: ") + std::to_string(index) + " not in " + std::to_string(bufferStartIndex) +
...
std::string UnbufferedTokenStream::getText(const misc::Interval &interval) {
    ...
    if (start < bufferStartIndex || stop > bufferStopIndex) {
        throw UnsupportedOperationException(
            std::string("interval ") + interval.toString() + " not in token buffer window: " + std::to_string(bufferStartIndex) +

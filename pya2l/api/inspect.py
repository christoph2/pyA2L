...
class CompuMethod(CachedBase):
    ...
    def __str__(self):
        names = (
            self.name, self.longIdentifier, self.conversionType, self.format, self.unit,
            self.coeffs, self.coeffs_linear, self.formula, self.tab, self.tab_verb, self.statusStringRef, self.refUnit
            )
        return """
CompuMethod {{
    name               = "{}";

...
TokenStreamRewriter::RewriteOperation::RewriteOperation(
    TokenStreamRewriter *outerInstance_, size_t index_, const std::string &text_
) :
    outerInstance(outerInstance_) {
    InitializeInstanceFields();
    this->index = index_;
...
std::string TokenStreamRewriter::ReplaceOp::toString() {
    ...
    return "<ReplaceOp@" + outerInstance->tokens->get(index)->getText() + ".." + outerInstance->tokens->get(lastIndex)->getText() +
           ":\"" + text + "\">";
}
...
void TokenStreamRewriter::rollback(const std::string &programName, size_t instructionIndex) {
    ...
    if (is.size() > 0) {
        _programs.insert({ programName,
                           std::vector<RewriteOperation *>(is.begin() + MIN_TOKEN_INDEX, is.begin() + instructionIndex) });
    }
}
...
void TokenStreamRewriter::replace(const std::string &programName, size_t from, size_t to, const std::string &text) {
    if (from > to || to >= tokens->size()) {
        throw IllegalArgumentException(
            "replace: range invalid: " + std::to_string(from) + ".." + std::to_string(to) +
    ...
    for (size_t i = 0; i < rewrites.size(); ++i) {
        ...
        for (auto *prevRop : prevReplaces) {
            ...
            if (prevRop->text.empty() && rop->text.empty() && !disjoint) {
                ...
                throw IllegalArgumentException(
                    "replace op boundaries of " + rop->toString() + " overlap with previous " + prevRop->toString()
                );
            }
        }
    }
    ...
    for (size_t i = 0; i < rewrites.size(); i++) {
        ...
        for (auto *rop : prevReplaces) {
            ...
            if (iop->index >= rop->index && iop->index <= rop->lastIndex) {
                throw IllegalArgumentException(
                    "insert op " + iop->toString() + " within boundaries of previous " + rop->toString()
                );
            }
        }
    }

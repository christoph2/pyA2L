...
struct Filenames {
  ...
  Filenames(const std::string &_a2l, const std::string &_aml,
            const std::string &_ifdata)
...
public:
  ...
  Preprocessor(const std::string &loglevel)
      : tmp_a2l(A2L_TMP, true), tmp_aml(AML_TMP), tmp_ifdata(IFDATA_TMP, true),
        a2l_token_writer(tmp_a2l), ifdata_builder{tmp_ifdata.handle()} {
  ...
  std::tuple<Filenames, LineMap, IfDataReader>
  process(const std::string &filename, const std::string &encoding) {
    _process_file(filename);
    return {m_filenames, line_map,
            IfDataReader{m_filenames.ifdata, ifdata_builder}};
  }
...
protected:
  void _process_file(const std::string &filename) {
    ...
    if (line_map.contains(abs_pth.string())) {
      throw std::runtime_error("[ERROR (pya2l.Preprocessor)]: Circular "
    ...
    if (file.is_open()) {
      std::cout
          << "[INFO (pya2l.Preprocessor)]: Preprocessing and tokenizing '" +
                 filename + "'."
          << std::endl;
      ...
      for (auto token : tokenizer(file)) {
        ...
        if (token.m_token_class == TokenClass::COMMENT) {
          ...
          if (include == true) {
            ...
            if (auto incl_file = locate_file(_fn, path.parent_path().string());
                incl_file.has_value()) {
              auto length = (end_line - start_line_number);
              update_line_map(abs_pth, line_offset, line_offset + length - 1,
                              start_line_number, end_line - 1);
              line_offset += length;

              std::cout << "[INFO (pya2l.Preprocessor)]: Including '" +
                               incl_file.value().string() + "'."
                        << std::endl;
              _process_file(incl_file.value().string());
            } else {
              throw std::runtime_error("[ERROR (pya2l.Preprocessor)]: Could "
      ...
      auto length = (end_line - start_line_number);
      update_line_map(abs_pth, line_offset, line_offset + length,
                      start_line_number, end_line);
      line_offset += length;
    } else {
  ...
  void update_line_map(const fs::path &path, std::uint64_t abs_start,
                       std::uint64_t abs_end, std::uint64_t rel_start,
                       std::uint64_t rel_end) {
  ...
  std::optional<fs::path> locate_file(const std::string &file_name,

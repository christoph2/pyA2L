...
const atn::ATN &Parser::getATNWithBypassAlts() {
    ...
    return *cache->map.insert(std::make_pair(std::vector<int32_t>(serializedAtn.begin(), serializedAtn.end()), std::move(atn)))
                .first->second;
}
...
tree::pattern::ParseTreePattern Parser::compileParseTreePattern(const std::string &pattern, int patternRuleIndex, Lexer *lexer) {
...
void Parser::setProfile(bool profile) {
    ...
    if (profile) {
        if (!is<atn::ProfilingATNSimulator *>(interp)) {
            setInterpreter(new atn::ProfilingATNSimulator(this)

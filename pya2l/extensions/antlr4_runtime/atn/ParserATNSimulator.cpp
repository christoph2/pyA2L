...
ParserATNSimulator::ParserATNSimulator(
    const ATN &atn, std::vector<dfa::DFA> &decisionToDFA, PredictionContextCache &sharedContextCache
) :
    ParserATNSimulator(nullptr, atn, decisionToDFA, sharedContextCache) {
}

ParserATNSimulator::ParserATNSimulator(
    Parser *parser, const ATN &atn, std::vector<dfa::DFA> &decisionToDFA, PredictionContextCache &sharedContextCache
) :
...
size_t ParserATNSimulator::execATN(
    dfa::DFA &dfa, dfa::DFAState *s0, TokenStream *input, size_t startIndex, ParserRuleContext *outerContext
) {
#if DEBUG_ATN == 1 || TRACE_ATN_SIM == 1
    std::cout << "execATN decision " << dfa.decision << ", DFA state " << s0->toString() << ", LA(1)==" << getLookaheadName(input)
...
dfa::DFAState *ParserATNSimulator::computeTargetState(dfa::DFA &dfa, dfa::DFAState *previousD, size_t t) {
    ...
    dfa::DFAState *D =
        new dfa::DFAState(std::move(reach)); /* mem-check: managed by the DFA or deleted below, "reach" is no longer valid now. */
...
size_t ParserATNSimulator::execATNWithFullContext(
    dfa::DFA &dfa, dfa::DFAState *D, ATNConfigSet *s0, TokenStream *input, size_t startIndex, ParserRuleContext *outerContext
) {
#if TRACE_ATN_SIM == 1
...
#if DEBUG_ATN == 1 || TRACE_ATN_SIM == 1
    std::cout << "computeStartState from ATN state " << p->toString() << " initialContext=" << initialContext->toString()
              << std::endl;
#endif
...
size_t ParserATNSimulator::getAltThatFinishedDecisionEntryRule(ATNConfigSet *configs) {
    ...
    for (const auto &c : configs->configs) {
        if (c->getOuterContextDepth() > 0 ||
            (c->state != nullptr && c->state->getStateType() == ATNStateType::RULE_STOP && c->context->hasEmptyPath())) {
            alts.add(c->alt);
        }
    }
    if (alts.size() == 0) {
...
BitSet ParserATNSimulator::evalSemanticContext(
    const std::vector<dfa::DFAState::PredPrediction> &predPredictions, ParserRuleContext *outerContext, bool complete
) {
    BitSet predictions;
...
bool ParserATNSimulator::evalSemanticContext(
    Ref<const SemanticContext> const &pred, ParserRuleContext *parserCallStack, size_t /*alt*/, bool /*fullCtx*/
) {
    return pred->eval(parser, parserCallStack);
}

void ParserATNSimulator::closure(
    Ref<ATNConfig> const &config, ATNConfigSet *configs, ATNConfig::Set &closureBusy, bool collectPredicates, bool fullCtx,
...
void ParserATNSimulator::closureCheckingStopState(
    Ref<ATNConfig> const &config, ATNConfigSet *configs, ATNConfig::Set &closureBusy, bool collectPredicates, bool fullCtx,
...
void ParserATNSimulator::closure_(
    Ref<ATNConfig> const &config, ATNConfigSet *configs, ATNConfig::Set &closureBusy, bool collectPredicates, bool fullCtx,
    int depth, bool treatEofAsEpsilon
) {
...
bool ParserATNSimulator::canDropLoopEntryEdgeInLeftRecursiveRule(ATNConfig *config) const {
    ...
    for (size_t i = 0; i < numCtxs; i++) {  // for each stack context
        ...
        if (returnStateTarget->getStateType() == ATNStateType::BLOCK_END && returnStateTarget->transitions.size() == 1 &&
...
Ref<ATNConfig> ParserATNSimulator::getEpsilonTarget(
    Ref<ATNConfig> const &config, const Transition *t, bool collectPredicates, bool inContext, bool fullCtx, bool treatEofAsEpsilon
) {
    switch (t->getTransitionType()) {
        ...
        case TransitionType::PRECEDENCE:
            return precedenceTransition(
                config, static_cast<const PrecedencePredicateTransition *>(t), collectPredicates, inContext, fullCtx
            );
...
Ref<ATNConfig> ParserATNSimulator::precedenceTransition(
    Ref<ATNConfig> const &config, const PrecedencePredicateTransition *pt, bool collectPredicates, bool inContext, bool fullCtx
) {
#if DFA_DEBUG == 1
    std::cout << "PRED (collectPredicates=" << collectPredicates << ") " << pt->getPrecedence() << ">=_p"
...
Ref<ATNConfig> ParserATNSimulator::predTransition(
    Ref<ATNConfig> const &config, const PredicateTransition *pt, bool collectPredicates, bool inContext, bool fullCtx
) {
#if DFA_DEBUG == 1
    std::cout << "PRED (collectPredicates=" << collectPredicates << ") " << pt->getRuleIndex() << ":" << pt->getPredIndex()
...
NoViableAltException ParserATNSimulator::noViableAlt(
    TokenStream *input, ParserRuleContext *outerContext, ATNConfigSet *configs, size_t startIndex, bool deleteConfigs
) {
...
void ParserATNSimulator::reportAttemptingFullContext(
    dfa::DFA &dfa, const antlrcpp::BitSet &conflictingAlts, ATNConfigSet *configs, size_t startIndex, size_t stopIndex
) {
...
void ParserATNSimulator::reportContextSensitivity(
    dfa::DFA &dfa, size_t prediction, ATNConfigSet *configs, size_t startIndex, size_t stopIndex
) {
#if DFA_DEBUG == 1 || RETRY_DEBUG == 1
...
void ParserATNSimulator::reportAmbiguity(
    dfa::DFA &dfa, dfa::DFAState * /*D*/, size_t startIndex, size_t stopIndex, bool exact, const antlrcpp::BitSet &ambigAlts,
    ATNConfigSet *configs
) {
#if DFA_DEBUG == 1 || RETRY_DEBUG == 1
    misc::Interval interval = misc::Interval((int)startIndex, (int)stopIndex);
    std::cout << "reportAmbiguity " << ambigAlts << ":" << configs << ", input=" << parser->getTokenStream()->getText(interval)
              << std::endl;
#endif

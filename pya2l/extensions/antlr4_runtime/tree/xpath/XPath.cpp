...
std::unique_ptr<XPathElement> XPath::getXPathElement(Token *wordToken, bool anywhere) {
    ...
    switch (wordToken->getType()) {
        ...
        case XPathLexer::STRING:
            if (ttype == Token::INVALID_TYPE) {
                throw IllegalArgumentException(
                    word + " at index " + std::to_string(wordToken->getStartIndex()) + " isn't a valid token name"
                );
            }
            if (anywhere)
        ...
        default:
            if (ruleIndex == -1) {
                throw IllegalArgumentException(
                    word + " at index " + std::to_string(wordToken->getStartIndex()) + " isn't a valid rule name"
                );
            }
            if (anywhere)

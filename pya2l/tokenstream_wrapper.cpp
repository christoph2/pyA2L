...
PYBIND11_MODULE(tokenstream, m) {
    py::class_<TokenReader>(m, "TokenReader")
        ...
        .def("LT", &TokenReader::LT, py::arg("k"))
        .def("seek", &TokenReader::seek, py::arg("k"))
        .def_property_readonly("index", &TokenReader::getIndex)
        .def("consume", &TokenReader::consume)
        .def("mark", &TokenReader::mark)
        .def("release", &TokenReader::release, py::arg("k"))
    ...
    py::class_<TokenFactory>(m, "TokenFactory").def("create", &TokenFactory::create);
}

...
Ref<const PredictionContext> getCachedContextImpl(
    ...
    if (parents.empty()) {
        ...
        updated = std::make_shared<ArrayPredictionContext>(
            std::move(parents), downCast<const ArrayPredictionContext *>(context.get())->returnStates
        );
        contextCache.put(updated);
    }
...
void getAllContextNodesImpl(
    const Ref<const PredictionContext> &context, std::vector<Ref<const PredictionContext>> &nodes,
...
size_t insertOrAssignNodeId(
    std::unordered_map<const PredictionContext *, size_t> &nodeIds, size_t &nodeId, const PredictionContext *node
) {
...
PredictionContext::PredictionContext(PredictionContext &&other) :
    _contextType(other._contextType), _hashCode(other._hashCode.exchange(0, std::memory_order_relaxed)) {
}
...
Ref<const PredictionContext> PredictionContext::fromRuleContext(const ATN &atn, RuleContext *outerContext) {
    ...
    auto parent = PredictionContext::fromRuleContext(
        atn, RuleContext::is(outerContext->parent) ? downCast<RuleContext *>(outerContext->parent) : nullptr
    );
...
Ref<const PredictionContext> PredictionContext::merge(
    Ref<const PredictionContext> a, Ref<const PredictionContext> b, bool rootIsWildcard, PredictionContextMergeCache *mergeCache
) {
    assert(a && b);
...
Ref<const PredictionContext> PredictionContext::mergeSingletons(
    Ref<const SingletonPredictionContext> a, Ref<const SingletonPredictionContext> b, bool rootIsWildcard,
...
Ref<const PredictionContext> PredictionContext::mergeRoot(
    Ref<const SingletonPredictionContext> a, Ref<const SingletonPredictionContext> b, bool rootIsWildcard
) {
    if (rootIsWildcard) {
        if (a == EMPTY) {  // * + b = *
...
Ref<const PredictionContext> PredictionContext::mergeArrays(
    Ref<const ArrayPredictionContext> a, Ref<const ArrayPredictionContext> b, bool rootIsWildcard,
    PredictionContextMergeCache *mergeCache
) {
    if (mergeCache) {
        auto existing = mergeCache->get(a, b);
...
std::vector<std::string> PredictionContext::toStrings(
    Recognizer *recognizer, const Ref<const PredictionContext> &stop, int currentState
) const {

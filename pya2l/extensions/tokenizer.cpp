...
std::vector<Token> split_by_new_line(std::string_view line,
                                     std::size_t start_line,
                                     std::size_t start_column) {
  auto set_line_numbers = [](LineNumbers &l, std::size_t sl, std::size_t sc,
  ...
  if (LAST_IDX < 1) {
    return std::vector<Token>{Token(
        ...
        LineNumbers(start_line, start_column, start_line, start_column), line)};
  }
  if (line.find('\n') == std::string::npos) {
    return std::vector<Token>{
        Token(TokenClass::WHITESPACE,
              LineNumbers(start_line, start_column, start_line,
                          start_column + line.length() - 1),
              line)};
  }
  ...
  do {
    ...
    if (cstr) {
      ...
      if (prev == -1) {
        if (pos > 0) {
          set_line_numbers(line_numbers, row, start_column, row,
                           start_column + pos - 1);
          result.emplace_back(WHITESPACE, line_numbers, line.substr(0, pos));
          set_line_numbers(line_numbers, row, start_column + pos, row,
                           start_column + pos);
        ...
        if ((pos - prev) > 1) {
          ...
          set_line_numbers(line_numbers, row, prev + length, row,
                           prev + length);
          result.emplace_back(WHITESPACE, line_numbers,
                              line.substr(prev + length + 1, 1));
          row++;
        } else {
...
auto split_single_line_comment(std::string_view line, std::size_t start_line,
                               std::size_t start_column) {
  ...
  if (pos > 0) {
    LineNumbers line_numbers{start_line, start_column, start_line,
                             start_column + pos - 1};
    result.emplace_back(REGULAR, line_numbers, line.substr(0, pos));
    line_numbers = {start_line, start_column + pos, start_line,
                    line.length() - pos};
    result.emplace_back(COMMENT, line_numbers,
                        line.substr(pos, line.length() - pos - 1));
    line_numbers = {start_line, line.length() - pos + 1, start_line,
                    line.length() - pos + 1};
    result.emplace_back(WHITESPACE, line_numbers, "\n");
  } else {
    LineNumbers line_numbers{start_line, start_column, start_line,
...
auto split_multi_line_comment(std::string_view line, std::size_t start_line,
                              std::size_t start_column, std::size_t end_line,
  ...
  if (start_pos > 0) {
    ...
    line_numbers = {start_line, start_column + start_pos, start_line,
                    line.length()};
    result.emplace_back(COMMENT, line_numbers,
                        line.substr(start_pos, line.length() - start_pos));
  } else {
    LineNumbers line_numbers = {start_line, start_column + start_pos, end_line,
                                end_column + 1};
...
Generator<TokenizerReturnType> tokenizer(std::basic_istream<char> &stream,
                                         bool supress_whitespace) {
  char ch{};
  ...
  const auto in_comment = [&comment_state]() noexcept {
    ...
    return (comment_state == SINGLE_LINE) || (comment_state == OPEN) ||
           (comment_state == MAY_CLOSE);
  };

  const auto in_string = [&string_state]() noexcept {
    return (string_state == StringStateType::IN_STRING) ||
  ...
  while (!stream.eof()) {
    ...
    if (stream.eof()) {
      ...
      if (current == CharClass::WHITESPACE) {
        ...
        for (const auto &elem :
             split_by_new_line(token[char_class_to_int(CharClass::WHITESPACE)],
                               start_line, start_column)) {
          co_yield {elem};
        }
      } else if (current == CharClass::REGULAR) {
        co_yield {Token(TokenClass::REGULAR, line_numbers,
                        token[char_class_to_int(current)])};
      }
    }
    column++;
    ...
    if (comment_state == CommentStateType::MAY_START) {
      ...
      if (ch == SLASH) {
        ...
        for (const auto &elem : split_multi_line_comment(
                 token[char_class_to_int(CharClass::REGULAR)], start_line,
                 start_column, line, column - 1)) {
          co_yield {elem};
        }
        ...
        if (in_string() || in_comment()) {
          ...
          if (token[char_class_to_int(CharClass::WHITESPACE)].length()) {
            if (!supress_whitespace) {
              LineNumbers line_numbers{start_line, start_column, line,
                                       column - 1};
              for (const auto &elem : split_by_new_line(
                       token[char_class_to_int(CharClass::WHITESPACE)],
                       start_line, start_column)) {
                co_yield {elem};
              }
            }
          ...
          auto line_numbers{
              LineNumbers(start_line, start_column, line, column - 1)};
          if (current == CharClass::REGULAR) {
            token[char_class_to_int(CharClass::REGULAR)].push_back(ch);
            if (!supress_whitespace) {
              if (token[char_class_to_int(CharClass::WHITESPACE)] == "\n") {
                line_numbers = {start_line, start_column, start_line,
                                start_column};
              } else {
            ...
            if (token[char_class_to_int(CharClass::REGULAR)] != "") {
              co_yield {Token(
                  string_class ? TokenClass::STRING : TokenClass::REGULAR,
                  line_numbers, token[char_class_to_int(CharClass::REGULAR)])};
            }
            string_class = false;
            token[char_class_to_int(CharClass::REGULAR)].clear();
          }
    ...
    if (ch == NL) {
      ...
      if ((string_state == StringStateType::IN_STRING) ||
          (string_state == StringStateType::MAY_CLOSE)) {
      ...
      if (comment_state == CommentStateType::SINGLE_LINE) {
        ...
        for (const auto &elem : split_single_line_comment(
                 token[char_class_to_int(CharClass::REGULAR)], start_line,
                 start_column)) {
          co_yield {elem};
        }

...
def test_flush_strategy(a2l_path, divisor, label):
    ...
    try:
        ...
        def patched_load(self, file_name, in_memory=False, local=False, remove_existing=False, loglevel="INFO", progress_bar=True):
            ...
            from pya2l.a2lparser import path_components, FakeRoot, update_tables
    ...
    if results:
        ...
        for file_label, file_results in by_file.items():
            ...
            for r in file_results:
                ...
                print(f"{r['strategy']:<20} {r['elapsed_sec']:>8.2f}s {r['peak_memory_mib']:>10.1f}MiB "
            ...
            print(f"  Best time:   {best_time['strategy']} ({best_time['elapsed_sec']:.2f}s, {((best_time['elapsed_sec'] - baseline_time) / baseline_time * 100):+.1f}%)")

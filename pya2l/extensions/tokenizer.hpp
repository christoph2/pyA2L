...
struct Token {
  ...
  Token(TokenClass token_type, const LineNumbers &line_numbers,
        std::string_view payload)
      : m_token_class{token_type}, m_line_numbers{line_numbers}, m_payload{
                                                                     payload} {
    set_token_type();
  }
...
private:
  static constexpr auto PAT_FLOAT =
      ctll::fixed_string{"^[+\\-]?[0-9]+\\.[0-9]*([eE][+\\-]?[0-9]+)?|\\.[0-9]+"
                         "([eE][+\\-]?[0-9]+)?|[0-9]+[eE][+\\-]?[0-9]+"};

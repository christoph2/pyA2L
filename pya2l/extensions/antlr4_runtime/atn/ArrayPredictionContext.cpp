...
ArrayPredictionContext::ArrayPredictionContext(
    std::vector<Ref<const PredictionContext>> parents, std::vector<size_t> returnStates
) :
...
bool ArrayPredictionContext::equals(const PredictionContext &other) const {
    ...
    const bool  sameSize = returnStates.size() == array.returnStates.size() && parents.size() == array.parents.size();
    if (!sameSize) {
        return false;
    }
    ...
    const bool   returnStateArraysEqual =
        std::memcmp(returnStates.data(), array.returnStates.data(), returnStates.size() * stateSizeBytes) == 0;
    if (!returnStateArraysEqual) {
        return false;
    }

    // stack of contexts is the same
    const bool parentCtxEqual = std::equal(parents.begin(), parents.end(), array.parents.begin(), predictionContextEqual);

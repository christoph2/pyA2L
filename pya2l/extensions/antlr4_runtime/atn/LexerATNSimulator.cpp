...
LexerATNSimulator::LexerATNSimulator(
    Lexer *recog, const ATN &atn, std::vector<dfa::DFA> &decisionToDFA, PredictionContextCache &sharedContextCache
) :
...
size_t LexerATNSimulator::execATN(CharStream *input, dfa::DFAState *ds0) {
    if (ds0->isAcceptState) {
        ...
        // ml: in Java code this method uses 3 params. The first is a member var of the class anyway (_prevAccept), so why pass it
        // here?
...
dfa::DFAState *LexerATNSimulator::getExistingTargetState(dfa::DFAState *s, size_t t) {
    ...
    if (t <= MAX_DFA_EDGE) {
        ...
        if (iterator != s->edges.end()) {
            std::cout << std::string("reuse state ") << s->stateNumber << std::string(" edge to ") << iterator->second->stateNumber
                      << std::endl;
        }
#endif
...
size_t LexerATNSimulator::failOrAccept(CharStream *input, ATNConfigSet *reach, size_t t) {
    if (_prevAccept.dfaState != nullptr) {
        accept(
            input, _prevAccept.dfaState->lexerActionExecutor, _startIndex, _prevAccept.index, _prevAccept.line, _prevAccept.charPos
        );
        ...
        for (size_t ti = 0; ti < n; ti++) {  // for each transition
            ...
            if (target != nullptr) {
                ...
                Ref<LexerATNConfig> config =
                    std::make_shared<LexerATNConfig>(downCast<const LexerATNConfig &>(*c), target, std::move(lexerActionExecutor));
...
void LexerATNSimulator::accept(
    CharStream *input, const Ref<const LexerActionExecutor> &lexerActionExecutor, size_t /*startIndex*/, size_t index, size_t line,
    size_t charPos
) {
#if LEXER_DEBUG_ATN == 1
    std::cout << "ACTION ";
...
bool LexerATNSimulator::closure(
    CharStream *input, const Ref<LexerATNConfig> &config, ATNConfigSet *configs, bool currentAltReachedAcceptState,
...
Ref<LexerATNConfig> LexerATNSimulator::getEpsilonTarget(
    CharStream *input, const Ref<LexerATNConfig> &config, const Transition *t, ATNConfigSet *configs, bool speculative,
        ...
        case TransitionType::ACTION:
            if (config->context == nullptr || config->context->hasEmptyPath()) {
                ...
                auto lexerActionExecutor = LexerActionExecutor::append(
                    config->getLexerActionExecutor(), atn.lexerActions[static_cast<const ActionTransition *>(t)->actionIndex]
                );

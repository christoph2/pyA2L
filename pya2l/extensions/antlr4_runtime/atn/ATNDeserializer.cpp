    ...
    switch (type) {
        ...
        default:
            throw IllegalArgumentException(
                "The specified lexer action type " + std::to_string(static_cast<size_t>(type)) + " is not valid."
            );
    }
}

ConstTransitionPtr edgeFactory(
    const ATN &atn, TransitionType type, size_t trg, size_t arg1, size_t arg2, size_t arg3,
...
std::unique_ptr<ATN> ATNDeserializer::deserialize(SerializedATNView data) const {
    ...
    for (ATNState *state : atn->states) {
        for (size_t i = 0; i < state->transitions.size(); i++) {
            ...
            ConstTransitionPtr returnTransition =
                std::make_unique<EpsilonTransition>(ruleTransition->followState, outermostPrecedenceReturn);
